# docker model reinstall-runner

Reinstall Docker Model Runner (Docker Engine only).

## Usage

```text
docker model reinstall-runner
```

## Options

| Option | Description |
|---|---|
| `--backend string` | Specify backend (llama.cpp\|vllm\|diffusers). Default: llama.cpp |
| `--debug` | Enable debug logging |
| `--do-not-track` | Do not track models usage in Docker Model Runner |
| `--gpu string` | Specify GPU support (none\|auto\|cuda\|rocm\|musa\|cann) (default "auto") |
| `--host string` | Host address to bind Docker Model Runner (default "127.0.0.1") |
| `--llama-server-version string` | Override the llama.cpp version to install on macOS/Windows |
| `--port uint16` | Docker container port for Docker Model Runner (default: 12434 for Docker Engine, 12435 for Cloud mode) |
| `--proxy-cert string` | Path to a CA certificate file for proxy SSL inspection |
| `--tls` | Enable TLS/HTTPS for Docker Model Runner API |
| `--tls-cert string` | Path to TLS certificate file (auto-generated if not provided) |
| `--tls-key string` | Path to TLS private key file (auto-generated if not provided) |
| `--tls-port uint16` | TLS port for Docker Model Runner (default: 12444 for Docker Engine, 12445 for Cloud mode) |
